...
import { FiShoppingCart, FiHeart, FiUser, FiMenu, FiX, FiSearch, FiMessageCircle, FiLogOut, FiSettings, FiPackage } from 'react-icons/fi'
...
export default function Navbar() {
  ...
  return (
    <header className={`sticky top-0 z-40 transition-all duration-300 ${scrolled ? 'bg-white/95 backdrop-blur-md shadow-sm' : 'bg-white'} border-b border-gray-100`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-2">
            <Link to="/wishlist" className="relative p-2 hover:bg-gray-100 rounded-xl transition-colors hidden md:flex">
                  ...
                  {profileOpen && (
                    <>
                      ...
                      >
                        ...
                        <div className="p-2">
                          <DropdownItem to="/profile" icon={<FiUser />} label="My Profile" onClick={() => setProfileOpen(false)} />
                          <DropdownItem to="/orders" icon={<FiPackage />} label="My Orders" onClick={() => setProfileOpen(false)} />
                          <DropdownItem to="/messages" icon={<FiMessageCircle />} label="Messages" onClick={() => setProfileOpen(false)} />
                          {isAdmin && <DropdownItem to="/admin" icon={<FiSettings />} label="Admin Panel" onClick={() => setProfileOpen(false)} />}
                          <button
            ...
            <button onClick={() => setMenuOpen(m => !m)} className="md:hidden p-2 hover:bg-gray-100 rounded-xl">
        ...
        {menuOpen && (
          ...
          >
            <div className="p-4 space-y-2">
              ...
              {navLinks.map(link => (
                <Link key={link.to} to={link.to} className="block px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 rounded-xl">
                  {link.label}
                </Link>
              ))}
              {!user && (
                <div className="flex gap-2 pt-2">
                  ...
                  <Link to="/register" className="flex-1 btn-primary text-sm text-center">Sign Up</Link>
                </div>
              )}
            </div>
          </motion.div>
